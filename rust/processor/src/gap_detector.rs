...
use crate::custom_processor::{CustomProcessor, CustomProcessorTrait};
...
pub async fn create_gap_detector_status_tracker_loop_for_custom_processor(
    gap_detector_receiver: AsyncReceiver<ProcessingResult>,
    custom_processor: CustomProcessor,
    starting_version: u64,
    gap_detection_batch_size: u64,
    ...
    info!(
        processor_name = processor_name,
        service_type = PROCESSOR_SERVICE_TYPE,
        "[Parser] Starting gap detector task for custom processor",
    );
